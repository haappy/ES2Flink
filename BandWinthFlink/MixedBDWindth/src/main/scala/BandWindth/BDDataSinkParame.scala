...
class BDDataSinkParame() extends RichSinkFunction[Stat]{
  private val LOG = LoggerFactory.getLogger(classOf[BDDataSinkParame])
  ...
  override def open(parameters: Configuration): Unit ={
    ...
    conn = DriverManager.getConnection("jdbc:mysql://172.16.5.250:3366/ecdn_bi?characterEncoding=UTF-8&serverTimezone=Asia/Shanghai"
                                      , "bi", "ppio@123")
    //conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ppio?characterEncoding=UTF-8&serverTimezone=Asia/Shanghai", "root", "12345678")
